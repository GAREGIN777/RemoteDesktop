...
class AdminSettingsFragment : Fragment() {
    ...
    fun showBecomeCameraModal() {
        val dialog = Dialog(requireActivity())
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE)
        dialog.setCancelable(true)
        dialog.setContentView(R.layout.dialog_admin_become_camera)

        val submitBtn: Button = dialog.findViewById(R.id.submit_camera);
        val denyBtn: Button = dialog.findViewById(R.id.deny_camera);
        ...
        submitBtn.setOnClickListener {
            it.isEnabled = false
            sharedUserData.updateRole(UserRole.USER, object : ParamChangeListener {
                override fun onResult(statusMessage: String, status: Boolean) {
                    if (status) {
                        goToMain()
                    } else {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.adminSettingsList.setNavigationItemSelectedListener {
            when (it.itemId) {
                R.id.admin_settings_become_camera -> {
                            // Do something when menu item 1 is clicked
                    showBecomeCameraModal()
                    true
                }
